...
than one would expect by chance. \is{Point-wise Mutual Information (PMI)} PMI (Point-wise Mutual Information)
...
A MWE can be characterised by its \is{mean span} \textsc{mean span}, that is,
...
\begin{equation*}
\resizebox{.85\hsize}{!}{$\mu^{'}_{\left\{\mathit{bite},\mathit{dog}\right\}}=\sqrt{\frac{\left(-4-\left(-0.6\right)\right)^2+\left(-4-\left(-0.6\right)\right)^2+%\left(3-\left(-0.6\right)\right)^2+%\left(-2-\left(-0.6\right)\right)^2+
...
Mean span & Stand-ard deviation\\%%\hline
\midrule
$[$back$]$$[$bite$]$ &
87 &
5.914 &
...
standard measures for collocation strength: \isi{frequency of collocation},
PMI, \isi{t-score} and \isi{LogDice}. Both the t-score and LogDice are used by the
